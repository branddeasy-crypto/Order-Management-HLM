...
import { Customer, Reputation, formatIDR } from "@/lib/types";
...
const REPUTATION_CONFIG: Record<Reputation, { label: string; badge: string; icon: string; dpSuggestion: string }> = {
  baru:     { label: "Baru",     badge: "bg-gray-100 text-gray-600",    icon: "🆕", dpSuggestion: "40-50%" },
  loyal:    { label: "Loyal",    badge: "bg-green-100 text-green-700",  icon: "⭐", dpSuggestion: "15-25%" },
  berisiko: { label: "Berisiko", badge: "bg-red-100 text-red-700",      icon: "⚠️", dpSuggestion: "50% atau tolak" },
};

type OrderStat = { customer_id: string; total: number; completed: number };
...
export default function CustomersPage() {
  ...
  async function load() {
    ...
    if (oRes.data) {
      ...
      for (const o of oRes.data as { customer_id: string; status: string }[]) {
        if (!statsMap[o.customer_id]) statsMap[o.customer_id] = { customer_id: o.customer_id, total: 0, completed: 0 };
        statsMap[o.customer_id].total++;
        if (o.status === "paid_off" || o.status === "shipped") statsMap[o.customer_id].completed++;
      }
      setOrderStats(Object.values(statsMap));
    }
    setLoading(false);
  }
  ...
  const statsByCustomer = useMemo(() => {
    const m: Record<string, OrderStat> = {};
    for (const s of orderStats) m[s.customer_id] = s;
    return m;
  }, [orderStats]);
  ...
  async function setReputation(c: Customer, reputation: Reputation) {
    await supabase.from("customers").update({ reputation }).eq("id", c.id);
    load();
  }

  function openDeposit(id: string) {
    if (depositCustomerId === id) { setDepositCustomerId(null); return; }
  ...
  async function saveDeposit(customer: Customer) {
    ...
    const { error } = await supabase.from("customers").update({ credit_balance: newBalance }).eq("id", customer.id);
  ...
  async function resetDeposit(customer: Customer) {
    if (!confirm(`Reset saldo deposit ${customer.whatsapp_name} ke Rp 0?`)) return;
  ...
  const countBerisiko = customers.filter(c => c.reputation === "berisiko").length;
  const countLoyal = customers.filter(c => c.reputation === "loyal").length;
    ...
    <div>
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">Data WhatsApp, alamat, penerima paket, grup, dan reputasi pembeli.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <div className="bg-gray-50 border border-gray-100 rounded-xl p-3">
          <div className="text-xs text-gray-400 mb-0.5">Total Customer</div>
          <div className="font-bold text-gray-700">{customers.length}</div>
        </div>
        <div className="bg-green-50 border border-green-100 rounded-xl p-3">
          <div className="text-xs text-green-500 mb-0.5">⭐ Loyal</div>
          <div className="font-bold text-green-700">{countLoyal} customer</div>
        </div>
        <div className="bg-red-50 border border-red-100 rounded-xl p-3">
          <div className="text-xs text-red-500 mb-0.5">⚠️ Berisiko</div>
          <div className="font-bold text-red-700">{countBerisiko} customer</div>
        </div>
        <div className="bg-purple-50 border border-purple-100 rounded-xl p-3">
          <div className="text-xs text-purple-500 mb-0.5">💜 Total Deposit</div>
          <div className="font-bold text-purple-700 text-sm">{formatIDR(totalDeposit)}</div>
        </div>
      </div>
      ...
      <div className="bg-pink-50 border border-pink-100 rounded-2xl p-5 mb-6">
        ...
        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <label className="text-sm flex flex-col gap-1">
            <span className="text-gray-600 font-medium text-xs">Reputasi Awal</span>
            <select className="border border-gray-200 rounded-lg px-3 py-2 bg-white text-sm focus:outline-none focus:border-pink-400 focus:ring-2 focus:ring-pink-100"
              value={form.reputation} onChange={(e) => setForm({ ...form, reputation: e.target.value as Reputation })}>
              <option value="baru">🆕 Baru (DP 40-50%)</option>
              <option value="loyal">⭐ Loyal (DP 15-25%)</option>
              <option value="berisiko">⚠️ Berisiko (DP 50% / tolak)</option>
            </select>
          </label>
        ...
        <table className="w-full text-sm">
          <thead className="text-left text-xs text-gray-400 uppercase tracking-wide">
            <tr>
              ...
              <th className="px-5 py-3">Reputasi</th>
              <th className="px-5 py-3">Riwayat Order</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td className="px-5 py-8 text-gray-400 text-center" colSpan={7}>👥 Belum ada customer. Tambah di atas!</td></tr>
            ) : customers.map((c) => {
              const rep = (c.reputation ?? "baru") as Reputation;
              const repConf = REPUTATION_CONFIG[rep];
              const stats = statsByCustomer[c.id];
              const completionRate = stats && stats.total > 0 ? Math.round((stats.completed / stats.total) * 100) : null;
              return (
                <>
                  <tr key={c.id} className={`border-t border-gray-50 transition-colors ${depositCustomerId === c.id ? "bg-purple-50/60" : "hover:bg-pink-50/40"}`}>
                    <td className="px-5 py-3">
                      <div className="font-medium text-gray-800">{c.whatsapp_name}</div>
                      <div className="text-xs text-gray-400">{c.whatsapp_number}</div>
                    </td>
                    <td className="px-5 py-3">
                      {c.whatsapp_group && (
                    ...
                    <td className="px-5 py-3">
                      <div className="flex flex-col gap-1">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium w-fit ${repConf.badge}`}>
                          {repConf.icon} {repConf.label}
                        </span>
                        <div className="flex gap-1">
                          {(["baru", "loyal", "berisiko"] as Reputation[]).map((r) => (
                            <button key={r} onClick={() => setReputation(c, r)}
                              className={`text-xs px-1.5 py-0.5 rounded transition-colors border ${rep === r ? "border-gray-400 bg-gray-100 text-gray-700" : "border-gray-200 text-gray-400 hover:border-gray-300"}`}
                              title={`Tandai sebagai ${REPUTATION_CONFIG[r].label}`}>
                              {REPUTATION_CONFIG[r].icon}
                            </button>
                          ))}
                        </div>
                        <div className="text-xs text-gray-400">DP: {repConf.dpSuggestion}</div>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                      {stats ? (
                        <div className="text-xs">
                          <div className="text-gray-600">{stats.total} order total</div>
                          <div className="text-green-600">{stats.completed} lunas/terkirim</div>
                          {completionRate !== null && (
                            <div className={`font-medium ${completionRate >= 80 ? "text-green-600" : completionRate >= 50 ? "text-amber-600" : "text-red-500"}`}>
                              {completionRate}% selesai
                            </div>
                          )}
                        </div>
                      ) : (
                        <span className="text-xs text-gray-300">Belum ada order</span>
                      )}
                    </td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3 text-gray-500 text-xs">{c.receiver_name}</td>
                    <td className="px-5 py-3 text-right whitespace-nowrap">
                      <button onClick={() => openDeposit(c.id)}
                      ...
                      <td colSpan={7} className="px-5 py-4 bg-purple-50/80">
                        <div className="max-w-xl">
                          <p className="text-xs font-semibold text-purple-700 mb-3 flex items-center gap-1.5">
                            💰 Catat Deposit Masuk - <span className="font-bold">{c.whatsapp_name}</span>
                            {(c.credit_balance ?? 0) > 0 && (
                              <span className="ml-1 text-purple-500 font-normal">(saldo saat ini: {formatIDR(c.credit_balance ?? 0)})</span>
                            )}
                          </p>
                          ...
                          <div className="flex gap-2 flex-wrap">
                            <button onClick={() => saveDeposit(c)} disabled={depositSaving}
